...
export default function AdminEquipos({ showToast, showConfirm, categoria }: AdminEquiposProps) {
  ...
  const availableTeams = allCategoryTeams.filter(t => !equipos.some(e => e.id === t.id));

  async function ensureTeamInLeague(teamId: string) {
    if (!relationEnabled) return;
    const { error } = await supabase.from("equipo_competiciones").upsert(
      [{ equipo_id: teamId, categoria, competicion: selectedLiga }],
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusyText(editingId ? "Guardando cambios equipo..." : "Creando equipo...");
    setBusyProgress(5);
    setLoading(true);
    ...
    try {
      ...
      if (escudoEquipo) {
        const processed = await processAndUploadImage(escudoEquipo, (percent) => {
      ...
      if (editingId) {
        ...
        const { data: inserted, error } = await supabase.from("equipos").insert([{
          nombre: nombreEquipo,
          escudo_url: url,
          categoria,
        }]).select("id").single();
  ...
  async function handleDeleteEquipo(id: string) {
    showConfirm(relationEnabled ? "¿Quitar equipo de esta liga?" : "¿Borrar equipo de librería?", async () => {
      if (relationEnabled) {
        await supabase.from("equipo_competiciones")
          .delete()
          .eq("equipo_id", id)
          .eq("categoria", categoria)
          .eq("competicion", selectedLiga);
  ...
  async function handleUpdateEscudo(id: string, file: File) {
    ...
    try {
      ...
      setBusyProgress(75);
      const fileName = `escudos/${uuidv4()}.webp`;
      const { data: uploadData, error: uploadError } = await supabase.storage.from("fotos").upload(fileName, processed);
      ...
      setBusyProgress(90);
      const { error: dbError } = await supabase.from("equipos").update({ escudo_url: pUrl.publicUrl }).eq("id", id);
      if (dbError) {
  ...
  return (
    <div className="card glass">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <div>
          ...
          <p style={{ color: "#a3a3a3", fontSize: "0.85rem" }}>
            Gestiona clubes por competición. Reutiliza equipos existentes sin duplicar.
          </p>
        </div>
        {editingId && (
          <button onClick={resetForm} className="btn-delete" style={{ padding: "0.4rem 1rem" }}>
            Cancelar Edición
          </button>
        )}
      </div>
      ...
      <div className="input-group" style={{ marginBottom: "1rem" }}>
        ...
        <select value={selectedLiga} onChange={(e) => setSelectedLiga(e.target.value)} disabled={loading || isFetching}>
          {ligaOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
        </select>
      </div>

      {relationEnabled && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 220px", gap: "0.8rem", marginBottom: "1.2rem" }}>
          <div className="input-group">
            <label>Añadir desde equipos ya existentes</label>
            <select value={selectedExistingId} onChange={(e) => setSelectedExistingId(e.target.value)} disabled={loading}>
              <option value="">Seleccionar equipo...</option>
              {availableTeams.map(t => <option key={t.id} value={t.id}>{t.nombre}</option>)}
            </select>
          </div>
          <div style={{ display: "flex", alignItems: "flex-end" }}>
            <button type="button" className="btn-primary" style={{ width: "100%" }} onClick={handleAddExistingTeam} disabled={loading || !selectedExistingId}>
              Añadir a liga
            </button>
          </div>
        </div>
      ...
      {!relationEnabled && (
        <p style={{ color: "#f59e0b", fontSize: "0.8rem", marginBottom: "1rem" }}>
          Tabla `equipo_competiciones` no existe. Modo clásico activo por categoría.
        </p>
      )}
      ...
      <form
        ...
        style={{ background: editingId ? "rgba(250, 204, 21, 0.05)" : "", padding: editingId ? "1.5rem" : "", borderRadius: "1rem", transition: "all 0.3s" }}
      >
      ...
      <div style={{ marginTop: "2.5rem" }}>
        <h4 style={{ marginBottom: "1rem", fontSize: "0.9rem", color: "#666", textTransform: "uppercase", letterSpacing: "1px" }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                <div style={{ position: "relative", width: "36px", height: "36px" }}>
                  <div style={{ width: "100%", height: "100%", background: "rgba(255,255,255,0.03)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                  ...
                  <label style={{ position: "absolute", bottom: "-6px", right: "-6px", background: "var(--primary)", color: "#000", borderRadius: "50%", width: "20px", height: "20px", display: "flex", alignItems: "center", justifyContent: "center", cursor: loading ? "not-allowed" : "pointer", border: "2px solid #000", opacity: loading ? 0.6 : 1 }}>
              ...
              <div style={{ display: "flex", gap: "15px" }}>
                <button disabled={loading} onClick={() => startEdit(e)} className="text-secondary" style={{ fontSize: "0.8rem", fontWeight: 600, background: "none", border: "none", color: "var(--primary)", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1 }}>Editar</button>
                <div style={{ width: "1px", height: "15px", background: "rgba(255,255,255,0.1)" }} />
                <button disabled={loading} onClick={() => handleDeleteEquipo(e.id)} className="text-red" style={{ fontSize: "0.8rem", fontWeight: 600, background: "none", border: "none", color: "#ef4444", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1 }}>
