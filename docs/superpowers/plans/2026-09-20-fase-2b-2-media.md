...
- [x] **Paso 1: Subir el límite del cuerpo de las acciones**
...
- [x] **Paso 4: Implementar la pieza de servidor**
...
- [x] **Paso 5: Implementar la pieza de cliente**
...
- [x] **Paso 6: Conectar `AdminEquipos`**
...
- [x] **Paso 7: Conectar `AdminPlayers`**
...
- [x] **Paso 8: Comprobar en el navegador y commit**
...
- [x] **Paso 7: Conectar `AdminStaff` y comprobar**
...
- [x] **Paso 6: Ejecutar las pruebas y conectar `AdminSponsors`**
...
- [x] **Paso 4: Implementar la consulta de ajustes**
...
- [x] **Paso 7: Conectar `AdminCartelAssets`**
...
- [x] **Paso 8: Conectar el escudo de `AdminShield`**
...
- [x] **Paso 9: Retirar el procesado de imagen del navegador**
...
- [x] **Paso 10: Prueba e2e de la media**
...
- [x] **Paso 11: Comprobar en el navegador**
...
- [x] **Paso 12: Puerta de calidad y commit**
...
- [x] `pnpm e2e` → `10 passed`.
- [x] `grep -rn "supabase.storage" apps` deja **una sola** aparición, en `components/admin/cartel/useCartelAssets.ts`, que es de 2B-3.
- [x] `grep -rn "supabase" apps/studio/components/admin/AdminEquipos.tsx apps/studio/components/admin/AdminPlayers.tsx apps/studio/components/admin/AdminStaff.tsx apps/studio/components/admin/AdminSponsors.tsx apps/studio/components/admin/AdminCartelAssets.tsx` sin resultados.
- [x] `AdminShield.tsx` **sí** sigue importando Supabase, y solo para `handleLogout`: se retira en 2C.
- [x] `apps/studio/lib/image-utils.ts` ya no existe y `browser-image-compression` no está en `package.json`.
- [x] La línea base de ESLint de la app no supera los **101 problemas**.
- [x] `git status --short` limpio y sin `data/` ni `.env.local`.
- [x] Ninguna imagen de las secciones migradas apunta a `supabase.co`; todas salen de `/media/`.
- [x] Las secciones de 2B-3 siguen intactas: `git diff --name-only` no toca `AdminJornadas`, `AdminActaImporter`, `AdminActaBatch`, `AdminJornadaImporter`, `useCartelForm` ni `useCartelAssets`.
...
- **Las acciones de guardado reescriben la fila entera.** Quien actualice solo una imagen tiene que reenviar el resto de campos del registro, o se vacían. `AdminPlayers` y `AdminStaff` ya lo hacen, y dos pruebas de `jugadores.test.ts` fijan el contrato. 2B-3 tiene el mismo patrón en el guardado de actas: cuidado ahí, donde el coste de perder datos es mucho mayor.
- **`supabase-queries.ts` es ya un nombre engañoso:** sus lecturas de temporadas, competiciones y equipos van contra SQLite. Renombrarlo a algo como `lib/consultas-ui.ts` es limpieza de 2C.
